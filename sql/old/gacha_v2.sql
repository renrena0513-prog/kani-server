-- 新ガチャ仕様: is_gacha_eligible のTEXT化 & gacha_weight追加 & draw_gacha_v2
...
DROP FUNCTION IF EXISTS public.draw_gacha_v2(text, text, text, integer);
...
BEGIN
    ...
    SELECT allowed_payment_types, coin_price INTO v_allowed_payments, v_config_coin_price
    ...
    IF p_payment_type = 'coin' THEN
        ...
        UPDATE public.profiles SET coins = coins - v_total_cost WHERE discord_user_id = v_target_user_id;
    ELSIF p_payment_type = 'gacha_ticket' THEN
        ...
        END IF;
        UPDATE public.profiles SET gacha_tickets = COALESCE(gacha_tickets, 0) - v_count WHERE discord_user_id = v_target_user_id;
    ELSIF p_payment_type = 'mangan_ticket' THEN
        ...
        END IF;
        UPDATE public.profiles SET mangan_tickets = COALESCE(mangan_tickets, 0) - v_count WHERE discord_user_id = v_target_user_id;
    END IF;
    ...
    FOR v_draw IN 1..v_count LOOP
        ...
        ELSE
            ...
            IF v_badge.sales_type = '変動型' THEN
                SELECT count(*) INTO v_circulation_count FROM public.user_badges_new WHERE badge_id = v_badge.id;
