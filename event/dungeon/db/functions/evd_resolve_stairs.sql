...
begin
    ...
    select coalesce(fbp.bonus_coins, 0)
     ...
     where fbp.profile_id = v_run.generation_profile_id
       ...
       and not (v_claimed_floor_bonuses @> jsonb_build_array(v_target_floor));
    ...
    update public.evd_game_runs
       set run_coins = run_coins + v_bonus,
           ...
           inventory_state = case
                when v_bonus > 0 then jsonb_set(
                    inventory_state,
                    array['flags', 'claimed_floor_bonuses'],
                    coalesce(inventory_state -> 'flags' -> 'claimed_floor_bonuses', '[]'::jsonb) || jsonb_build_array(v_target_floor),
                    true
                )
