CREATE OR REPLACE FUNCTION public.poker_bet_settle(p_event_id uuid, p_winner_team_name text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
    ...
    SELECT EXISTS (
        SELECT 1 FROM jsonb_array_elements(v_event.teams) t WHERE t->>'team_name' = p_winner_team_name
    ) INTO v_team_valid;
    ...
    FOR v_bet IN SELECT * FROM poker_finals_bets WHERE event_id = p_event_id LOOP
        IF v_bet.team_name = p_winner_team_name THEN
    ...
    UPDATE poker_finals_predictions
        SET status = 'settled', winner_team_name = p_winner_team_name, settled_at = now(), settled_by = v_admin_id
        WHERE id = p_event_id;
