...
BEGIN
    ...
    FOR i IN 1..v_n LOOP
        FOR j IN 1..v_n LOOP
            ...
            IF v_p2 > 0 THEN
                v_raw_odds := round(LEAST(GREATEST(v_settings.payout_rate / v_p2, v_settings.min_odds), v_settings.max_odds), 1);
            ELSE
                v_raw_odds := v_settings.max_odds;
            END IF;
    ...
    INSERT INTO poker_finals_predictions
        (title, status, teams, trio_odds, trifecta_odds, place_odds, exacta_odds, quinella_odds, created_by, payout_rate, min_odds, max_odds)
    VALUES
        (COALESCE(NULLIF(trim(p_title), ''), '決勝卓予想'), 'open', p_teams, v_trio, v_trifecta, v_place, v_exacta, v_quinella, v_admin_id,
