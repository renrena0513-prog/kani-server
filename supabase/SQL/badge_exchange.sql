...
-- 管理者：レシピ作成（素材は別途追加）
drop function if exists public.admin_create_badge_exchange(uuid, jsonb, boolean);
...
grant execute on function public.admin_create_badge_exchange(uuid, text, integer, jsonb, boolean) to authenticated;
